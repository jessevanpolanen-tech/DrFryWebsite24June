...
//
// The token is just the base64url-encoded email — enough to make casual
// tampering pointless. For a stricter guarantee, HMAC it with a secret.
...
export default async function handler(req) {
  ...
  try {
    ...
    email = decodeURIComponent(escape(atob(b64)));
  } catch {}

  if (!email || !email.includes('@')) {
    return html('That unsubscribe link looks invalid. Email jesse@drfry.nl and we\'ll remove you by hand.', 400);
  }
  ...
  return html(`You're unsubscribed. <b>${escapeHtml(email)}</b> won't receive any further outreach from Dr. Fry.`);
}
